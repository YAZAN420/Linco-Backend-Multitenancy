...
export class ExamRandomResponseMapper {
  ...
  toResponseFromPrisma(examAttempt: RandomExam): RandomExamResponseDto {
    return new RandomExamResponseDto(
      ...
      examAttempt.questions.map((question) =>
        this.questionsBankResponseMapper.toResponseFromPrisma(question),
      ),
  ...
  toResponseManyFromPrisma(exams: RandomExam[]): RandomExamResponseDto[] {
